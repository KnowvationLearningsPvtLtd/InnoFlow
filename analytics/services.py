...
class AnalyticsService:
    ...
    def node_performance_stats(workflow_id=None):
        # Example: slowest nodes, most failing nodes, memory usage
        qs = WorkflowAnalytics.objects.all()
        if workflow_id:
            qs = qs.filter(workflow_id=workflow_id)
        # Suppose WorkflowAnalytics has a node field or join with Node
        # This is a placeholder; adjust as per your schema
        return qs.values('workflow', 'node').annotate(
            avg_execution_time=Avg('execution_time'),
            total_errors=Sum('error_count'),
